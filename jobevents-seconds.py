...
df = sql_context.sql("select round(timestamp/(1000000)) as timestamp, count(jobID) as numberOfJob from dataFrame group by round(timestamp/(1000000)) order by timestamp")
schema_df = ["time", "numberOfJob"]
df.toPandas().to_csv('results/jobevents-seconds.csv', index=False, header=None)
sc.stop()
