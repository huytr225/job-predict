...
df = sql_context.sql("select round((timestamp/1000000)/86400) as timestamp, count(jobID) as numberOfJob from dataFrame group by round((timestamp/1000000)/86400) order by timestamp")
schema_df = ["time", "numberOfJob"]
df.toPandas().to_csv('results/jobevents-days.csv', index=False, header=None)
sc.stop()
